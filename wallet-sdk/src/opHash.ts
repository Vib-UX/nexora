...
const USEROP_TYPEHASH = keccak256(
  toBytes(
    "UserOp(address sender,uint256 nonce,address target,uint256 value,bytes callData,uint256 callGasLimit,uint256 validUntil,uint8 policyTag,uint16 verifierScheme)",
  ),
);
...
export function structHash(op: UserOp): Hex {
  const calldataHash = keccak256(op.callData);
  return keccak256(
    encodeAbiParameters(
      parseAbiParameters(
        "bytes32, address, uint256, address, uint256, bytes32, uint256, uint256, uint8, uint16",
      ),
      [
        USEROP_TYPEHASH,
        op.sender,
...
export function padU8(v: number): Hex {
