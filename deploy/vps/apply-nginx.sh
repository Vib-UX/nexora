...
echo "  Snippets: /etc/nginx/snippets/nexora-*.map.conf, blockchain.nexorapq.in.servers.conf"
...
echo "  2) sudo certbot --nginx -d blockchain.nexorapq.in -d ws.blockchain.nexorapq.in"
echo "  3) curl -sS https://blockchain.nexorapq.in -X POST -H 'Content-Type: application/json' \\"
